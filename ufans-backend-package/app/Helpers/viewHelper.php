...
function user_order_status_formatted($status) {

    $status_list = [

                // ORDER_NONE => tr('ORDER_NONE'),
                ORDER_PLACED => tr('ORDER_PLACED'),
                ORDER_SHIPPED => tr('ORDER_SHIPPED'),
                ORDER_DELIVERD => tr('ORDER_DELIVERD'),
                ORDER_CACELLED => tr('ORDER_CACELLED'),
                ORDER_PACKED => tr('ORDER_PACKED'),
                

                ];

    return isset($status_list[$status]) ? $status_list[$status] : tr('ORDER_NONE');
}
function user_order_status_stylbtn_formatted($status) {

    $status_list = [

                ORDER_NONE => "badge badge-primary",
                ORDER_PLACED => "badge badge-secondary",
                ORDER_SHIPPED => "badge badge-dark",
                ORDER_DELIVERD => "badge badge-success",
                ORDER_CACELLED => "badge badge-danger",
                ORDER_PACKED => "badge badge-warning",
...
function content_creator_type_formatted($type) {

    $list = [DEFAULT_USER => tr('DEFAULT_USER'), CONTENT_CREATOR => tr('CONTENT_CREATOR')];

    return $list[$type] ?? tr('CONTENT_CREATOR');
