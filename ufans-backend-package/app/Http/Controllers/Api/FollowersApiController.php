...
class FollowersApiController extends Controller
{
    ...
    public function user_suggestions(Request $request) {
        ...
        try {
            ...
            if ($request->premium) {
                ...
                $base_query = $base_query->where('user_account_type', USER_PREMIUM_ACCOUNT);
            }
    ...
    public function chat_users(Request $request) {
        ...
        try {
            ...
            $base_query = $total_query = \App\Models\ChatUser::where('from_user_id', $request->id)->whereNotIn('to_user_id',$blocked_user_ids)->whereHas('fromUser')->whereHas('toUser')->orderBy('chat_users.updated_at', 'desc');
            ...
            foreach ($chat_users as $key => $chat_user) {
                ...
                $chat_messages = \App\Models\ChatMessage::where(function($query) use ($request,$chat_user){
                        $query->where('chat_messages.from_user_id', $request->id);
                        $query->where('chat_messages.to_user_id', $chat_user->to_user_id);
                    })->orWhere(function($query) use ($request,$chat_user){
                        $query->where('chat_messages.from_user_id', $chat_user->to_user_id);
                        $query->where('chat_messages.to_user_id', $request->id);
                    })
                    ->latest()
                    ->first();
                ...
                if($chat_messages) {
                    ...
                    if ($chat_messages->created_at->isToday()) {
                    
                        $chat_user->time_formatted = common_date($chat_messages->created_at, $this->timezone, 'h:i A');

                    } else {
                        $chat_user->time_formatted = common_date($chat_messages->created_at, $this->timezone, 'd M Y');  
                    }
