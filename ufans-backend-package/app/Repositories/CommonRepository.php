...
class CommonRepository {
    ...
    public static function chat_messages_list_response($chat_messages, $request) {
        ...
        $chat_messages = $chat_messages->map(function ($chat_message, $key) use ($request) {
            ...
            $chat_message->asset_file = $chat_message->file_type = '';
            ...
            if($chat_message->is_file_uploaded) {
                ...
                $chat_message->asset_file = $chat_assets ? $chat_assets->asset_file : '';

                $chat_message->blur_file = $chat_assets ? $chat_assets->blur_file : '';
    ...
    * @method posts_single_response()
    ...
    public static function chat_messages_asset_single_response($chat_message, $request) {
        ...
        if($chat_message->is_file_uploaded) {
            ...
            $chat_asset_payment = \App\Models\ChatAssetPayment::where('from_user_id', $chat_message->from_user_id)->where('to_user_id',$chat_message->to_user_id)->where('chat_message_id', $chat_message->id)->where('status', PAID)->count();

            $chat_message->is_user_paid = $chat_asset_payment ? YES : NO;
        }

        $chat_message->chat_message_status = $chat_message->from_user_id == $request->id ? CHAT_MESSAGE_STATUS_SENT : CHAT_MESSAGE_STATUS_RECEIVED;

        $chat_message->unsetRelation('fromUser')->unsetRelation('toUser');
    ...
    * @method chat_messages_response()
    ...
    public static function chat_messages_response($chat_messages, $request) {
    ...
    public static function create_default_user_login_session($request, $user_id) {
        ...
        $user_login_session->device_unique_id = $user_id;
    ...
    /**
     * @method chat_assets_list_response()
     *
     * @uses Format the chat response
     *
     * @created Bhawya N
    ...
    public static function chat_assets_list_response($chat_assets, $request) {
        ...
        $chat_assets = $chat_assets->map(function ($chat_asset, $key) use ($request) {
            ...
            $chat_asset->asset_file = '';

            $chat_message = \App\Models\ChatMessage::find($chat_asset->chat_message_id);

            $payment_info = self::chat_user_payment_check($chat_message, $request);

            $is_user_needs_pay = $payment_info->is_user_needs_pay ?? NO; 

            if($is_user_needs_pay == NO) {

                $chat_asset->asset_file = $chat_asset->file;

            } else {

                $chat_asset->asset_file = $chat_asset->blur_file;

            }

            $chat_asset->is_user_needs_pay = $is_user_needs_pay;

            $chat_asset->payment_text = $payment_info->payment_text ?? '';

            $chat_asset->chat_message = $chat_message->message ?? '';

            $chat_asset->chat_message_status = $chat_message->from_user_id == $request->id ? CHAT_MESSAGE_STATUS_SENT : CHAT_MESSAGE_STATUS_RECEIVED;

            $chat_asset->amount_formatted = formatted_amount(Setting::get('is_only_wallet_payment') ? $chat_message->token :$chat_message->amount);
