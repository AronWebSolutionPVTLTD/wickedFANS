...
class AdminUserController extends Controller
{
    ...
    public function users_index(Request $request) {
        ...
        if($request->search_key) {
            ...
            $search_user_ids = \App\Models\User::where('users.name','LIKE','%'.$request->search_key.'%')
                    ...
                    // ->orWhere('users.is_content_creator','LIKE','%'.$request->search_key.'%')
                    ->pluck('id');
        ...
        if($request->document_status != '') {
         ...
         if($request->is_content_creator != '') {

            if($request->is_content_creator == CONTENT_CREATOR){

                $base_query = $base_query->whereIn('users.is_content_creator',[CONTENT_CREATOR]);

                
            }else{

                $base_query = $base_query->where('users.is_content_creator',$request->is_content_creator);
        ...
        //     $base_query = $base_query->where('users.is_content_creator',$request->status);
    ...
    public function user_upgrade_account(Request $request) {
        ...
        try {
            ...
            $rules = [
                ...
                'first_name' => 'nullable|regex:/^\S*$/u',
                'route_number' => 'nullable|regex:/^\S*$/u',
                'last_name' => 'nullable|regex:/^\S*$/u',
                'swift_code' => 'nullable|regex:/^\S*$/u',
            ];
      ...
      try{
        ...
        $followings =  $base_query->paginate($this->take);
         ...
         foreach($followings as $following){
            ...
            $following->following_count = \App\Models\Follower::where('user_id',$user->id)->count();
    ...
    public function block_users_index(Request $request) {
        ...
        $block_users = $base_query->paginate($this->take);
         ...
         foreach($block_users as $block_user){
            ...
            $block_user->block_user_count = \App\Models\BlockUser::where('user_id',$user->id)->count();
    ...
    public function chat_asset_payments(Request $request) {
        ...
        if($request->user_id) {
            $base_query = $base_query->where('from_user_id',$request->user_id)
    ...
    public function billing_accounts_index(Request $request) {
        ...
        $billing_accounts = $base_query->paginate($this->take);

        return view('admin.users.bank_details')
                    ->with('page', 'users')
                    ->with('sub_page', 'users-view')
                    ->with('user', $user)
                    ->with('billing_accounts', $billing_accounts);
    }
    ...
    public function chat_asset_payment_excel(Request $request) {
     ...
     /**
     * @method chat_asset_payments_send_invoice
     *
    ...
    public function chat_asset_payments_send_invoice(Request $request) {

        try {

            $chat_asset_payment = \App\Models\ChatAssetPayment::where('id', $request->chat_asset_payment_id)->first();
            ...
            $user = \App\Models\User::find($chat_asset_payment->from_user_id);
            ...
            $email_data['chat_asset_payments'] =  $chat_asset_payment ?? "";

            $email_data['amount'] = formatted_amount($chat_asset_payment->user_amount) ?? 0;

            if(Setting::get('is_only_wallet_payment')) {
                
                $email_data['amount'] = formatted_amount($chat_asset_payment->user_token) ?? 0;
            }

            $email_data['user'] = $user ?? '';

            $email_data['chat_assets'] = $chat_asset_payment->chatAssets ?? '';

            $email_data['to_user'] = $chat_asset_payment->toUser ?? '';

            $email_data['subject'] =  tr('Chat_asset_payment_invoice_message')." ".Setting::get('site_name');

            $email_data['page'] = "emails.users.chat_payment_invoice";
     ...
     /**
     * @method live_video_payments_send_invoice
     *
     * @uses to send user invoice request details based on request id
     *
     * @created Sakthi
     ...
     **/
     ...
     public function live_video_payments_send_invoice(Request $request) {
        ...
        try {
            ...
            if(!$live_video_payment) {
                ...
                throw new Exception(tr('live_video_payment_not_found'), 101);
            }

            $user = \App\Models\User::find($live_video_payment->user_id);
            ...
            $email_data['live_video_payments'] =  $live_video_payment ?? "";

            $email_data['user'] = $user ?? '';

            $email_data['live_video'] = $live_video_payment->videoDetails ?? '';

            $email_data['amount'] = formatted_amount($live_video_payment->user_amount) ?? 0;

            if(Setting::get('is_only_wallet_payment')) {
                
                $email_data['amount'] = formatted_amount($live_video_payment->user_token) ?? 0;
            }

            $email_data['subject'] =  tr('live_video_payment_invoice_message')." ".Setting::get('site_name');
