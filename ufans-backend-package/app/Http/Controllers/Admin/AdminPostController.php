...
use App\Models\User, App\Models\Hashtag, App\Models\Category, App\Models\CategoryDetail, App\Models\PostBookmark,App\Models\Order;
...
class AdminPostController extends Controller
{
    ...
    public function posts_index(Request $request) {
        ...
        $amount = Setting::get('is_only_wallet_payment') ? 'token' : 'amount';
            ...
            if($request->search_key) {
                ...
                $post_ids = Post::when($request->user_id, function($query) use ($request){
                ...
                $base_query = $base_query->whereIn('id', $post_ids);
       }
     ...
     public function hashtags_edit(Request $request) {
    ...
    */
    ...
    public function posts_create() {
        ...
        $users = \App\Models\User::Approved()->where('is_content_creator',CONTENT_CREATOR)->get();
    ...
    */
    ...
    public function orders_index(Request $request) {
     ...
     $search_key = $request->search_key;
         ...
         if($request->search_key) {
                ...
                $order_ids = Order::when($request->user_id, function($query) use ($request){
                           ...
                           })->orWhereHas('deliveryAddressDetails', function($query) use ($request) {

                               return $query->where('name', "LIKE", "%" . $request->search_key . "%");

                            })->orWhere('unique_id', "LIKE", "%" . $request->search_key . "%")->pluck('id');                

                $base_query = $base_query->whereIn('id', $order_ids);
       }
    ...
    public function post_bookmarks_index(Request $request) {
        ...
        foreach($post_bookmarks as $post_bookmark){

            $post_bookmark->post_bookmark_count = \App\Models\PostBookmark::where('post_id',$user->id)->count();
    ...
    public function post_bookmarks_delete(Request $request) {
        ...
        try {
            ...
            if($post_bookmark->delete()) {
                ...
                return redirect()->route('admin.bookmarks.index',['page'=>$request->page,'user_id'=>$request->user_id])->with('flash_success', tr('bookmark_deleted_success'));   
    ...
    public function fav_users(Request $request) {
        ...
        $base_query = \App\Models\FavUser::whereHas('favUser')->Approved()->orderBy('fav_users.created_at', 'desc');
        ...
        foreach($fav_users as $fav_user){
            ...
            $fav_user->fav_user_count = \App\Models\FavUser::where('user_id',$user->id)->count();
    ...
    public function post_likes(Request $request) {
        ...
        $base_query = \App\Models\PostLike::with('postUser')->Approved()->orderBy('post_likes.created_at', 'desc');

         if($request->search_key) {

        $liked_post_ids = PostLike::when($request->user_id, function($query) use ($request){
                         ...
                         })->orWhereHas('postUser', function($query) use ($request) {
            ...
            $base_query = $base_query->whereIn('id', $liked_post_ids);
    ...
    public function post_payments_send_invoice(Request $request) {
        ...
        try {
            ...
            $email_data['amount'] = formatted_amount($post_payment->user_amount) ?? 0;

            if(Setting::get('is_only_wallet_payment')) {
                
                $email_data['amount'] = formatted_amount($post_payment->user_token) ?? 0;
            }
    ...
    public function report_posts_delete(Request $request) {
        ...
        try {
            ...
            if($request->post_id){
                ...
                return redirect()->with('flash_success',tr('report_delete_success'));   
    ...
    */
    public function hashtags_save(Request $request) {
        ...
        try {
            ...
            $rules = [
                'name' => $request->hashtag_id ? 'nullable|max:255|unique:hashtags,name,'.$request->hashtag_id.',id' : 'required|regex:/^\S*$/u|max:255|unique:hashtags,name,NULL,id',
