...
use App\Models\UserProduct, App\Models\User, App\Models\Post,App\Models\PostPayment;
...
use App\Models\{ UserWalletPayment, UserWithdrawal };
...
class AdminRevenueController extends Controller
{
    ...
    public function main_dashboard() {
        ...
        if(Setting::get('is_only_wallet_payment')){
            ...
            $data->total_content_creators = \App\Models\User::where('is_content_creator', CONTENT_CREATOR)->count();
    ...
    public function post_payments(Request $request) {
        ...
        if($request->search_key) {
            ...
            $post_payment_ids = PostPayment::when($request->user_id, function($query) use ($request){
                      ...
                      })->orwhereHas('userOrder', function($query) use ($request) {

                           return $query->where('unique_id', "LIKE", "%" . $request->search_key . "%");

                      })->orwhereHas('postDetails', function($query) use ($request) {
            ...
            $base_query = $base_query->whereIn('id', $post_payment_ids);
        }
    ...
    public function order_payments(Request $request) {
      ...
      if($request->search_key) {
            ...
            $order_payment_ids = \App\Models\OrderPayment::when($request->user_id, function($query) use ($request){
            ...
            $base_query = $base_query->whereIn('id', $order_payment_ids);
        }
    ...
    public function user_withdrawals(Request $request) {
       ...
       if($request->status) {
        ...
        if($request->search_key) {
            ...
            $withdrawal_ids = \App\Models\UserWithdrawal::when($request->user_id, function($query) use ($request){
            ...
            $base_query = $base_query->whereIn('id', $withdrawal_ids);
        }
    ...
    public function user_withdrawals_paynow(Request $request) {
        ...
        try {
            ...
            $user_wallet_payment = UserWalletPayment::find($user_withdrawal->user_wallet_payment_id);
            ...
            if(!$user_wallet_payment) {
                ...
                throw new Exception(tr('user_withdrawal_wallet_payment_not_found'),101);
            ...
            if($user_withdrawal->save()) {
                ...
                $user_wallet_payment->update(['status' => USER_WALLET_PAYMENT_PAID]);
    ...
    public function subscription_payments_index(Request $request) {
        ...
        if($request->search_key) {
            ...
            $subscription_payment_ids = \App\Models\SubscriptionPayment::when($request->user_id, function($query) use ($request){
                          ...
                          // ->orWhere('plan_type', "LIKE", "%" . $request->search_key . "%")

                      ->pluck('id');                

            $base_query = $base_query->whereIn('id', $subscription_payment_ids);
        }
    ...
    public function subscription_payments_send_invoice(Request $request) {
        ...
        try {
            ...
            $email_data['amount'] = formatted_amount($subscription_payment->user_amount) ?? 0;

            if(Setting::get('is_only_wallet_payment')) {
                
                $email_data['amount'] = formatted_amount($subscription_payment->user_token) ?? 0;
            }
    ...
    public function order_payment_excel(Request $request) {
     ...
     /**
     * @method user_tip_payments_send_invoice
     *
     * @uses to send user invoice request details based on request id
     *
     * @created Sakthi
    ...
    public function user_tip_payments_send_invoice(Request $request) {

        try {

            $user_tip_payment = \App\Models\UserTip::where('id', $request->user_tip_id)->first();
            
            if(!$user_tip_payment) {

                throw new Exception(tr('user_tip_payment_not_found'), 101);
            }

            $user = \App\Models\User::find($user_tip_payment->user_id);
            ...
            $email_data['user_tip_payments'] =  $user_tip_payment ?? "";

            $email_data['amount'] = formatted_amount($user_tip_payment->user_amount) ?? 0;

            if(Setting::get('is_only_wallet_payment')) {
                
                $email_data['amount'] = formatted_amount($user_tip_payment->user_token) ?? 0;
            }

            $email_data['user'] = $user ?? '';

            $email_data['subscriptions'] = $user_tip_payment->userSubscription ?? '';

            $email_data['subject'] =  tr('tip_payment_invoice_message')." ".Setting::get('site_name');

            $email_data['message'] =  tr('user_tip_payment_message',$user_tip_payment->plan_text_formatted)." ".$user->name;
            
            $email_data['page'] = "emails.users.user_tip_payments_invoice";
            ...
            return redirect()->route('admin.user_tips.index')->with('flash_error', $e->getMessage());
     ...
     /**
     * @method order_payments_send_invoice
     *
     * @uses to send user invoice request details based on request id
     *
     * @created Sakthi
     ...
     **/
     ...
     public function order_payments_send_invoice(Request $request) {
        ...
        try {
            ...
            if(!$order_payment) {
                ...
                throw new Exception(tr('order_payment_not_found'), 101);
            }

            $user = \App\Models\User::find($order_payment->user_id);
            ...
            $email_data['order_payments'] =  $order_payment ?? "";

            $email_data['user'] = $user ?? '';

            $email_data['subscriptions'] = $order_payment->userSubscription ?? '';

            $email_data['subject'] =  tr('order_payment_invoice_message')." ".Setting::get('site_name');

            $email_data['message'] =  tr('order_payment_message',$order_payment->plan_text_formatted)." ".$user->name;
            
            $email_data['page'] = "emails.users.order_payments_invoice";
            ...
            return redirect()->route('admin.orders.payments')->with('flash_error', $e->getMessage());
