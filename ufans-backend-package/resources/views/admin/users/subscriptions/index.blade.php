            ...
            <div class="card user-subscription-sec">
                ...
                <div class="card-header border-bottom border-gray">
                    ...
                    <div class="heading-elements">
                        ...
                        <a href="{{ route('admin.subscription_payment.excel',['from_user_id'=>Request::get('from_user_id'),'search_key'=>Request::get('search_key'),'status'=>Request::get('status'),'file_format'=>'.csv']) }}" class="btn btn-primary resp-mrg-btm-xs">{{tr('Export_to_csv')}}</a>

                        <a href="{{ route('admin.subscription_payment.excel',['from_user_id'=>Request::get('from_user_id'),'search_key'=>Request::get('search_key'),'status'=>Request::get('status'),'file_format'=>'.xls']) }}" class="btn btn-primary resp-mrg-btm-xs">{{tr('Export_to_xls')}}</a>

                        <a href="{{ route('admin.subscription_payment.excel',['from_user_id'=>Request::get('from_user_id'),'search_key'=>Request::get('search_key'),'status'=>Request::get('status'),'file_format'=>'.xlsx']) }}" class="btn btn-primary resp-mrg-btm-xs">{{tr('Export_to_xlsx')}}</a>
                        ...
                        <table id="dataTable" class="table table-bordered table-striped display nowrap margin-top-10">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('admin.users.view' , ['user_id' => $subscription->from_user_id])}}"> {{ Str::limit($subscription->from_username ,20)?:tr('not_available')}}
                                        </a>
                                    </td>

                                    <td><a href="{{route('admin.users.view' , ['user_id' => $subscription->to_user_id])}}"> {{ Str::limit($subscription->to_username,20) ?:tr('not_available') }}</a></td>
